...
async function tradingAccessCheck(
  ...
  const [currencies, balance, historyV4] =
    await Promise.all([
      authenticatedCloudbetGet(env, currenciesUrl),
      authenticatedCloudbetGet(env, balanceUrl),
      authenticatedCloudbetGet(env, historyV4Url)
    ]);

  const currenciesAuthenticated = currencies.status === 200;
  const balanceAuthenticated = balance.status === 200;
  const historyV4Authenticated = historyV4.status === 200;
  const tradingReadAuthenticated = historyV4Authenticated;

  const access =
    balanceAuthenticated && tradingReadAuthenticated
  ...
  const availableCurrencies =
    Array.isArray(currencies?.data?.currencies)
      ? currencies.data.currencies
      : [];

  const usdtListed = availableCurrencies.includes("USDT");

  const bets =
    Array.isArray(historyV4?.data?.bets)
      ? historyV4.data.bets
      : [];
  ...
  return json({
    ...
    configured_currency: "USDT",
    access,

    checks: {
      account_currencies: {
        method: "GET",
        endpoint: "/pub/v1/account/currencies",
        ok: currencies.ok,
        http_status: currencies.status,
        elapsed_ms: currencies.elapsed_ms,
        authenticated: currenciesAuthenticated,
        usdt_listed: usdtListed,
        currencies: availableCurrencies,
        response: currencies.data ?? currencies.raw ?? null,
        error: currencies.error ?? null
      },

      account_balance_usdt: {
        method: "GET",
        endpoint: "/pub/v1/account/currencies/USDT/balance",
        ok: balance.ok,
        http_status: balance.status,
        elapsed_ms: balance.elapsed_ms,
        authenticated: balanceAuthenticated,
        currency: "USDT",
        response: balance.data ?? balance.raw ?? null,
        error: balance.error ?? null
      },

      trading_history_v4: {
        method: "GET",
        endpoint: "/pub/v4/bets/history?limit=1&offset=0",
        ok: historyV4.ok,
        http_status: historyV4.status,
        elapsed_ms: historyV4.elapsed_ms,
        authenticated: historyV4Authenticated,
        bets_count: bets.length,
        response: historyV4.data ?? historyV4.raw ?? null,
        error: historyV4.error ?? null
      }
    },

    summary: {
      account_api_authenticated:
        currenciesAuthenticated || balanceAuthenticated,
      usdt_available: usdtListed || balanceAuthenticated,
      usdt_balance_endpoint_ok: balanceAuthenticated,
      trading_v4_history_get_ok: historyV4Authenticated,
      trading_read_access_ok: tradingReadAuthenticated,
      api_bets_returned: bets.length
    },

    interpretation: {
      purpose:
        "Confirm Account API access and Trading API read authentication using GET /pub/v4/bets/history.",
      trading_history:
        historyV4Authenticated
          ? "HTTP 200 confirms the v4 Trading API history request was accepted. An empty bets array can be valid when there are no API-placed bets in history."
          : "The v4 Trading API history request was not accepted with HTTP 200.",
      no_bet_placed: true
    },
